...
**
*/
static WCHAR * UTF8ToWchar(const char* name)
{
    /*WCHAR*    bstrWide;
    int     nLength;
    nLength = MultiByteToWideChar(CP_UTF8, 0, name, strlen(name) + 1, NULL, NULL);
	bstrWide = (WCHAR*)malloc(nLength);// SysAllocStringLen(NULL, nLength);
    MultiByteToWideChar(CP_UTF8, 0, name, strlen(name) + 1, bstrWide, nLength);
    return bstrWide;
	*/
	...
	int     src_length = src_length = strlen(name);
	int length = MultiByteToWideChar(CP_UTF8, 0, name, src_length, 0, 0);
	WCHAR *output_buffer = (WCHAR*)malloc((length + 1) * sizeof(WCHAR));
	if (output_buffer) {
		MultiByteToWideChar(CP_UTF8, 0, name, src_length, output_buffer, length);
...
static WRes InFile_OpenChar(CSzFile *p, const char *name)
{
#ifdef USE_WINDOWS_FILE
    return InFile_OpenW(p, UTF8ToWchar(name));
#else
...
#else
static void GetAttribString(UInt32 unused1, Bool unused2, char *s)
{
    s[0] = '\0';
}
#endif
...
int main7z(int numargs, char *args[], Progress7z progress7z)
{
    ...
    if (InFile_OpenChar(&archiveStream.file, args[2]))
    {
    ...
    if (res == SZ_OK)
    {
        ...
        if (extractCommand && numargs >= 4) {
            outDirLen = strlen(args[3]);
            if (args[3][outDirLen - 1] != '/') {
                outDirLen++;
            }
        }
        ...
        if (res == SZ_OK)
        {
            ...
            for (i = 0; i < db.db.NumFiles; i++)
            {
                ...
                if (listCommand == 0 && f->IsDir && !fullPaths)
                    continue;
                len = SzArEx_GetFileNameUtf16(&db, i, NULL) + outDirLen;
                
                if (len > tempSize)
                {
                    const size_t TEMP_SIZE_MIN = 128;
                    SzFree(NULL, temp);
                    tempSize = len > TEMP_SIZE_MIN ? len : TEMP_SIZE_MIN;
                    ...
                    if (outDirLen) {
                        printf("alloc temp and copy outDir\n");
						char *p = args[3];
#ifdef USE_WINDOWS_FILE 
						int length = MultiByteToWideChar(CP_UTF8, 0, p, strlen(p) + 1, NULL, NULL);
						WCHAR * wp = UTF8ToWchar(p);
                        UInt16 *q = temp;
                        while (*wp) {
                            *q++ = *wp++;
                        }
                ...
                SzArEx_GetFileNameUtf16(&db, i, temp + outDirLen);
                if (listCommand)
                {
                ...
                else
                {
                    res = SzArEx_Extract(&db, &lookStream.s, i,
                                         ...
                                         &allocImp, &allocTempImp, progress7z);
                    if (res != SZ_OK)
                        break;
                }
                if (!testCommand)
                {
                    ...
                    if (extractCommand == 1 && fullPaths == 1) {
                        CBuf buf;
                        SRes res;
                        Buf_Init(&buf);
                        res = Utf16_To_Char(&buf, destPath, 0);
//                        const char * destPathChar = buf.data;
//                        int len = strlen(destPathChar);
//                        if (len >= 5) {
//                            if (destPath[len - 3] == 'l'
//                                && destPath[len - 2] == 'u'
//                                && destPath[len - 1] == 'a') {
